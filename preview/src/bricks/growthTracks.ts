  ...
  ai: {
    ...
    stages: [
      ...
      },
      {
        ...
        why: 'Real users ask about material the model was never trained on, and the agent’s steps need to be inspectable.',
      ...
      },
      {
        ...
        add: ['temporal'],
        links: [{ first: 'langgraph', second: 'temporal', kind: 'recipe', note: 'Authored for this stage: LangGraph owns agent state within a step; Temporal owns retries and resumption across steps.' }],
      ...
      },
      {
        ...
        links: [
          { first: 'langchain', second: 'temporal', kind: 'tension', note: 'In this design both layers try to own the same control loop.' },
        ...
        summary: 'Each new control part seemed useful on its own.',
        why: 'Tools were layered on top of the old ones instead of replacing or bounding them.',
        watch: 'Three parts claim control and Trust is still empty. Remove one, or assign boundaries as in Scale.',
      },
    ],
  },
  data: {
    ...
    stages: [
      {
        ...
        watch: 'No quality gates: bad rows flow straight into models and dashboards.',
      },
      {
      ...
      },
      {
        ...
        add: ['datahub', 'trino'],
        summary: 'A catalog for discovery and access, a query engine for wider serving.',
        why: 'More teams and more consumers need to find, trust, and query the same data.',
        watch: 'Catalog coverage decays unless every dataset has a named owner.',
      },
      {
    ...
    stages: [
      {
        ...
        why: 'The cheapest way to get an agent doing scheduled work.',
      ...
      },
      {
        ...
        watch: 'Function time limits and cold starts now constrain long workflows.',
      },
      {
        id: 'scale', horizon: 'Year 1', name: 'Hardened',
        add: ['kubernetes', 'opentelemetry', 'vault'], remove: ['lambda'],
        summary: 'Long-running workers, standard telemetry, managed secrets.',
      ...
      },
      {
        ...
        watch: 'No traces and no retries across a fleet: one failure now repeats everywhere.',
      },
    ],
  },
}
