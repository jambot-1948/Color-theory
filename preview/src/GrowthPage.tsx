...
import { editionTiers, readBuild, recipeLinks, recordedLinks, verdictCopy, type EditionId } from './bricks/buildModel'
...
import type { WorkshopTool } from './workshopData'
...
function stageModel(edition: EditionId, stage: GrowthStage) {
  ...
  const tools = ids.map(find).filter((tool): tool is WorkshopTool => Boolean(tool))
  const removedTools = removed.map(find).filter((tool): tool is WorkshopTool => Boolean(tool))
  const analysis = analyzeStack(tools, data)
  const recipeCaution = analysis.recipe?.patternIds.some(id => data.patterns.find(pattern => pattern.id === id)?.type === 'anti-pattern')
  const links = [...(stage.links ?? []), ...recordedLinks(tools), ...(recipeCaution ? [] : recipeLinks(tools, analysis.recipe))]
  const reading = readBuild(edition, tools, links, { caution: stage.caution || recipeCaution, gaps: stage.missing })
  const bricks = sceneBricks({ edition, data, tools, links, ghostHues: reading.gaps, removed: removedTools, newIds: added })
  return { data, tools, added, removedTools, reading, bricks, recipe: analysis.recipe }
}
...
function StageDetail({ edition, stage }: { edition: EditionId, stage: GrowthStage }) {
  const { data, tools, bricks, reading, added, removedTools, recipe } = stageModel(edition, stage)
  ...
  return <section className="gr-detail" aria-live="polite">
    ...
    <div className="gr-detail-copy">
      ...
      <div className="gr-changes">
        {added.map(id => { const tool = tools.find(item => item.id === id); const info = tool && hue(tool.primaryHue); return tool && info ? <div key={id} className="gr-change is-add"><Plus size={14} /><i style={{ background: info.hex }} /><span><strong>{tool.name}</strong><small>{info.name} · {tool.category}</small></span></div> : null })}
        {removedTools.map(tool => { const info = hue(tool.primaryHue); return <div key={tool.id} className="gr-change is-remove"><Minus size={14} /><i style={{ background: info?.hex }} /><span><strong>{tool.name}</strong><small>Taken off the model</small></span></div> })}
      </div>
      ...
      <a className="gr-open" href={`?blend=${tools.map(tool => tool.id).join(',')}${editionInfo[edition].href}`}>Open this model in the assembly <ArrowRight size={15} /></a>
    </div>
  </section>
}
