...
function PartGlyph({ role, color, y }: { role: string, color: string, y: number }) {
  const top = y + 17
  if (['interface', 'experience'].includes(role)) return <g><rect x="89" y={top + 5} width="43" height="27" rx="2" fill={color} /><rect x="94" y={top + 9} width="33" height="18" rx="1" fill="#fff" opacity=".88" /><rect x="86" y={top + 32} width="49" height="4" rx="1" fill={color} opacity=".7" /></g>
  ...
  if (['logic', 'service'].includes(role)) return <g><rect x="89" y={top + 4} width="43" height="29" rx="1" fill={color} opacity=".13" stroke={color} /><path d={`M 95 ${top + 11} H 115 V ${top + 26} H 125 M 115 ${top + 18} H 125`} fill="none" stroke={color} strokeWidth="3" strokeLinecap="square" /><circle cx="95" cy={top + 11} r="3" fill={color} /><circle cx="125" cy={top + 18} r="3" fill={color} /><circle cx="125" cy={top + 26} r="3" fill={color} /></g>
  ...
  if (['ingest', 'invocation', 'tools', 'delivery'].includes(role)) return <g><path d={`M 89 ${top + 7} H 113 V ${top + 15} H 130 V ${top + 31} H 89 Z`} fill={color} opacity=".8" /><path d={`M 98 ${top + 22} H 125 M 118 ${top + 16} L 125 ${top + 22} L 118 ${top + 28}`} fill="none" stroke="#fff" strokeWidth="3" /></g>
  ...
  if (['store', 'state', 'context', 'data'].includes(role)) return <g>{[0, 1, 2].map(index => <ellipse key={index} cx="110" cy={top + 10 + index * 10} rx="21" ry="7" fill={color} opacity={.45 + index * .2} stroke="#fff" />)}</g>
  if (['serve', 'scaling', 'runtime', 'platform'].includes(role)) return <g><rect x="89" y={top + 12} width="17" height="22" fill={color} /><rect x="110" y={top + 5} width="17" height="29" fill={color} opacity=".75" /><path d={`M 94 ${top + 19} H 101 M 115 ${top + 12} H 122`} stroke="#fff" strokeWidth="2" /></g>
  if (['observe', 'observability', 'evidence', 'operations'].includes(role)) return <g><path d={`M 89 ${top + 20} Q 110 ${top - 2} 132 ${top + 20} Q 110 ${top + 43} 89 ${top + 20}`} fill={color} opacity=".18" stroke={color} strokeWidth="2" /><circle cx="110" cy={top + 20} r="7" fill={color} /></g>
  if (['govern', 'security', 'permissions', 'trust'].includes(role)) return <g><path d={`M 90 ${top + 5} L 110 ${top + 1} L 131 ${top + 5} V ${top + 22} Q 126 ${top + 31} 110 ${top + 37} Q 94 ${top + 31} 90 ${top + 22} Z`} fill={color} opacity=".25" stroke={color} strokeWidth="2" /><path d={`M 101 ${top + 20} L 108 ${top + 26} L 121 ${top + 13}`} fill="none" stroke={color} strokeWidth="3" /></g>
