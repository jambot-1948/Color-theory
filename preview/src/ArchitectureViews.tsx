...
const journey = [
  ...
  { title: 'Retrieve', place: 'Application to external service', detail: 'The Agents SDK can call an explicitly implemented retrieval tool. That tool queries Pinecone for relevant indexed material; the connection is not automatic.' },
  { title: 'Generate', place: 'Application to external service', detail: 'The agent runtime sends the question and selected context to the OpenAI model API. The precise prompts and data sent need review.' },
...
export default function ArchitectureViews({ view }: { view: 'map' | 'journey' }) {
  ...
  return <div className="av-map">
    ...
    <div className="av-boundaries"><div><ArrowRight size={15} /><span>Keep API credentials on the backend, never in the browser.</span></div><div><ArrowRight size={15} /><span>Authentication, observability, failures, and deployment are open decisions.</span></div></div>
